...
def test_diff():
    ...
    session = Session(location=TEST_DIR, dataset=data_m)
    ...
    session = Session(location=TEST_DIR, dataset=data_dict, inplace=True)
...
def test_chg():
    data_m = dummy_df(freq="M")
    session = Session(location=TEST_DIR, dataset=data_m, inplace=True)
    ...
    data_q2 = dummy_df(freq="Q-DEC")
    data_dict = {"data_q1": data_q1, "data_q2": data_q2}
    session = Session(location=TEST_DIR, dataset=data_dict)
...
def test_decompose():
    ...
    session = Session(location=TEST_DIR, dataset=df[["Real"]])
    ...
    session = Session(location=TEST_DIR, dataset=df[["Real"]], inplace=True)
    ...
    assert std["Real"] >= std["Seas"]
    assert std["Real"] >= std["Trend"]
    session = Session(location=TEST_DIR, dataset={"data1": df[["Real"]],
                                                  "data2": df[["Real"]]})
    trend, seas = session.decompose(flavor="both", trading=True,
...
def test_base_index():
    ...
    session = Session(location=TEST_DIR, dataset=data, inplace=True)
