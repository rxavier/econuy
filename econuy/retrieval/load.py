...
def load_dataset(name: str, data_dir: Union[str, Path, None] = None, skip_cache: bool = False, safe_overwrite: bool = True) -> "Dataset":  # noqa: F821
...
def check_updated_dataset(original: "Dataset", new: "Dataset") -> None:  # noqa: F821
    ...
    assert shortened_original.notna().sum().sum() == shortened_new.notna().sum().sum(), "Datasets have different number of missing values"
    try:
        pd.testing.assert_series_equal(shortened_original.mean(), shortened_new.mean(), atol=0, rtol=0.05)
    except AssertionError as exc:
        raise AssertionError("Datasets have different means") from exc
    try:
        pd.testing.assert_series_equal(shortened_original.std(), shortened_new.std(), atol=0, rtol=0.05)
    except AssertionError as exc:
