...
def reqs_pipfile_lock(
    pipfile_lock: Union[str, os.PathLike, None] = None, exclude: Optional[List[str]] = None
):
    if exclude is None:
        exclude = []
    ...
    return [
        package_name
        for package_name in lock_data.get("default", {}).keys()
        if package_name not in exclude
    ]


packages = find_packages(".", exclude=["*.test", "*.test.*"])
pipfile_lock_requirements = reqs_pipfile_lock(exclude=["psycopg2", "statsmodels"])
...
setup(
    ...
    extras_require={"pgsql": ["psycopg2==2.8.5"]},
    packages=packages,
